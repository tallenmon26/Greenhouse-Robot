...
MISSING_LINE_THRESHOLD = 15 # How many frames of no line before we swap cameras
    ...
    while not should_quit:
        ...
        for i in range(num_cams):
            ...
            if i == active_cam_idx and in_depth is not None and in_rgb is not None:
                ...
                # 1. Convert the BGR image slice to HSV
                hsv_roi = cv2.cvtColor(line_roi_slice, cv2.COLOR_BGR2HSV)
                
                # 2. Define the lower boundary of red (Hue 0-10)
                lower_red_1 = np.array([0, 100, 100])
                upper_red_1 = np.array([10, 255, 255])
                mask1 = cv2.inRange(hsv_roi, lower_red_1, upper_red_1)
                
                # 3. Define the upper boundary of red (Hue 160-180)
                lower_red_2 = np.array([160, 100, 100])
                upper_red_2 = np.array([180, 255, 255])
                mask2 = cv2.inRange(hsv_roi, lower_red_2, upper_red_2)
                
                # 4. Combine both masks to get all red pixels
                thresh = cv2.bitwise_or(mask1, mask2)
                
                # SHOW THE MATH: This creates a second, black-and-white window.
                cv2.imshow("Binary Mask (What OpenCV Sees)", thresh)
                ...
                # Explicitly mapping the USB indexes to your physical cameras
                FRONT_CAM_INDEX = 1
                REAR_CAM_INDEX = 0
